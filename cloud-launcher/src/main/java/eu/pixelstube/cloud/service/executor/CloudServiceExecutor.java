    ...
    public void start() {
        ...
        try {
            ...
            if (cloudService.getCloudGroup().getTemplate().getType().equals(TemplateType.TEMPLATE)) {
                ...
                if (!CloudLauncher.getInstance().getFileManager().folderExist("templates/" + cloudService.getGroupName())) {
                    CloudLauncher.getInstance().getFileManager().createFolder("templates/" + cloudService.getGroupName());
                }

                CloudLauncher.getInstance().getFileManager().deleteFile("templates/" + cloudService.getGroupName(), "server.properties");
                CloudLauncher.getInstance().getFileManager().deleteFile("templates/" + cloudService.getGroupName(), "config.yml");
                CloudLauncher.getInstance().getFileManager().deleteFile("templates/" + cloudService.getGroupName(), "cloud.json");
                CloudLauncher.getInstance().getFileManager().deleteFile("templates/" + cloudService.getGroupName(), "bukkit.yml");
                CloudLauncher.getInstance().getFileManager().deleteFile("templates/" + cloudService.getGroupName(), "spigot.yml");
                CloudLauncher.getInstance().getFileManager().deleteFile("templates/" + cloudService.getGroupName(), "velocity.toml");

                CloudLauncher.getInstance().getFileManager().deleteFile("templates/" + cloudService.getGroupName() + "/plugins", "cloud-plugin.jar");

                for (ThunderModule module : CloudLauncher.getInstance().getModuleHandler().getModules()) {
                    CloudLauncher.getInstance().getFileManager().deleteFile("templates/" + cloudService.getGroupName() + "/plugins", module.getName() + ".jar");
                }

                CloudLauncher.getInstance().getFileManager().copyDirectory("templates/" + cloudService.getGroupName(), "temp/" + cloudService.getServiceIdName());
            }
